...
class ChatRealtimeHub:
    ...
    async def publish_to_users(self, user_ids: list[int], payload: dict[str, Any]) -> None:
        unique_ids = sorted(set(int(x) for x in user_ids if x))
        for user_id in unique_ids:
            await self._publish_to_user(user_id, payload)

    async def _publish_to_user(self, user_id: int, payload: dict[str, Any]) -> None:
        async with self._lock:
