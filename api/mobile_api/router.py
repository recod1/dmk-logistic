...
def _mark_route_received(db: Session, route: Route, user_id: int) -> bool:
    if route.assigned_user_id != user_id:
        return False
    if route.status not in {"new", "process"}:
        return False
    if route.driver_received_at is not None:
        return False
    route.driver_received_at = datetime.now(timezone.utc)
    db.add(route)
    return True


def _mark_routes_received(db: Session, user_id: int, routes: list[Route]) -> None:
    changed = False
    for route in routes:
        if _mark_route_received(db, route, user_id):
            changed = True
    if changed:
        db.commit()
...
def get_active_route(
    ...
    if route is None:
        return {"route": None}
    if _mark_route_received(db, route, current_user.id):
        db.commit()
        db.refresh(route)
    return {"route": _route_snapshot(db, route)}


@router.get("/v1/mobile/routes/prefetch")
def prefetch_driver_routes(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_driver),
) -> dict:
    """Full assigned-route payloads for background cache. Marks them received on the phone."""
    if _recalculate_driver_routes_completion(db, current_user.id):
        db.commit()
    all_routes = _driver_routes_for_user(db, current_user.id)
    assigned = [route for route in all_routes if route.status in {"new", "process"}]
    _mark_routes_received(db, current_user.id, assigned)
    active = pick_active_route(
        [DriverRouteState(id=item.id, status=item.status, created_at=item.created_at) for item in all_routes]
    )
    return {
        "items": [_route_snapshot(db, route) for route in assigned],
...
@router.get("/v1/mobile/routes/{route_id:path}")
...
@router.post("/v1/mobile/routes/{route_id:path}/accept")
def accept_route(
    route_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_driver),
    ...
    _mark_route_received(db, route, current_user.id)
    db.commit()
    db.refresh(route)
