...
def delete_salary(
    ...
    s = _get_salary_or_404(db, salary_id)
    att_rows = list(
        db.scalars(select(SalaryChatAttachment).where(SalaryChatAttachment.salary_id == salary_id)).all()
    )
    root = _mobile_upload_root()
    for att in att_rows:
        try:
            full_path = (root / att.storage_path).resolve()
            full_path.relative_to(root)
            if full_path.is_file():
                full_path.unlink()
        except Exception:
            pass
    db.execute(delete(SalaryChatRead).where(SalaryChatRead.salary_id == salary_id))
    db.execute(delete(SalaryChatAttachment).where(SalaryChatAttachment.salary_id == salary_id))
    db.execute(delete(SalaryChatMessage).where(SalaryChatMessage.salary_id == salary_id))
    db.delete(s)
    db.commit()
    return {"ok": True, "id": salary_id}
