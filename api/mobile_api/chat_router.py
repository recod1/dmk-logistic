...
@router.get("/v1/chat/routes/{route_id:path}/messages")
def list_messages(
    route_id: str,
...
@router.get("/v1/chat/unread-summary")
def unread_summary_mine(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> dict:
    if current_user.role_code == RoleCode.DRIVER.value:
        allowed = [str(x) for x in db.scalars(select(Route.id).where(Route.assigned_user_id == current_user.id)).all()]
        if not allowed:
            return {"items": []}
        route_filter = RouteChatMessage.route_id.in_(allowed)
    elif not _is_manager(current_user):
        return {"items": []}
    else:
        route_filter = true()

    last_subq = (
        select(RouteChatRead.route_id, RouteChatRead.last_read_message_id)
        .where(RouteChatRead.user_id == current_user.id)
        .subquery()
    )
    rows = db.execute(
        select(RouteChatMessage.route_id, func.count())
        .select_from(RouteChatMessage)
        .outerjoin(last_subq, last_subq.c.route_id == RouteChatMessage.route_id)
        .where(
            route_filter,
            RouteChatMessage.user_id != current_user.id,
            RouteChatMessage.id > func.coalesce(last_subq.c.last_read_message_id, 0),
        )
        .group_by(RouteChatMessage.route_id)
    ).all()
    return {"items": [{"route_id": str(route_id), "unread_count": int(count)} for route_id, count in rows if int(count) > 0]}


@router.post("/v1/chat/routes/{route_id:path}/messages", status_code=status.HTTP_201_CREATED)
def send_message(
    route_id: str,
...
@router.post("/v1/chat/routes/{route_id:path}/attachments", status_code=status.HTTP_201_CREATED)
